  ...
  blocks: [
    {
      ...
      concepts: [
        ...
        { id: "c2", description: 'Cena, "compartida"', typeLabel: "Compra compartida", amountMinor: "40000", purchaseAmountMinor: "100000", outstandingMinor: "0" },
      ],
      payments: [],
    },
  ],
...
describe("buildStatementCsv", () => {
  it("opens with a UTF-8 BOM and the expected header row", () => {
    ...
    expect(csv.slice(1).split("\r\n")[0]).toBe("Fecha,Concepto,Tipo,Mensualidad,Importe del periodo,Pendiente total,Estado,Moneda");
  });
  ...
  it("carries the exact per-period amount and marks an MSI concept with its installment progress", () => {
    ...
    expect(macbookLine).toContain("MSI,5 de 12,$666.67,$666.67,Pendiente,MXN");
  });
