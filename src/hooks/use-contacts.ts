...
function useInvalidatePeople() { const client = useQueryClient(); return () => Promise.all([client.invalidateQueries({ queryKey: ["contacts"] }), client.invalidateQueries({ queryKey: ["contact"] }), client.invalidateQueries({ queryKey: ["contact-receivables"] }), client.invalidateQueries({ queryKey: ["contact-activity"] }), client.invalidateQueries({ queryKey: ["contact-period"] }), client.invalidateQueries({ queryKey: ["contact-statement"] }), client.invalidateQueries({ queryKey: ["contact-installments"] }), client.invalidateQueries({ queryKey: ["accounts"] }), client.invalidateQueries({ queryKey: ["transactions"] })]); }
...
export function useContactStatement(id?: string) { return useQuery({ queryKey: ["contact-statement", id], queryFn: id ? () => contactService.statement(id) : skipToken }); }
