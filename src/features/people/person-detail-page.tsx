import { ArrowLeft, CreditCard, Landmark, Pencil, PiggyBank, Plus, WalletCards } from "lucide-react";
...
export function PersonDetailPage() {
  ...
  if (contact.isLoading || receivables.isLoading || activity.isLoading || period.isLoading) return <LoadingState label="Cargando persona" />;
  if (contact.isError || receivables.isError || activity.isError || period.isError) return <ErrorState message={toUserMessage(contact.error ?? receivables.error ?? activity.error ?? period.error)} onRetry={() => void contact.refetch()} />;
