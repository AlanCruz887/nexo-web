    ...
    <PageTransition>
      <div className="space-y-7">
        ...
        <section className="space-y-3">
          ...
          {advancedOpen ? <div className="grid gap-3 rounded-2xl bg-muted/70 p-4 sm:grid-cols-2 lg:grid-cols-3">
        ...
        {accounts.isError || movements.isError ? <ErrorState message={toUserMessage(accounts.error ?? movements.error)} onRetry={() => void movements.refetch()} /> : movements.isLoading || accounts.isLoading ? <LoadingState label="Cargando movimientos" /> : movements.data?.length ? <MovementList movements={movements.data} onSelect={setSelectedEventId} /> : <EmptyState action={activeAccounts.length ? <Button onClick={() => openMovement("expense")}>Registrar movimiento</Button> : undefined} description={activeAccounts.length ? "Prueba otro filtro o registra tu primera entrada o salida." : "Agrega una cuenta antes de registrar actividad."} title="No hay movimientos aquí" />}
      </div>
