...
const baselineOptions: Array<{ value: CardBaselinePolicy; title: string; description: string }> = [
  ...
  { value: "specific_date", title: "Desde una fecha específica", description: "Nexo contará los movimientos a partir de esa fecha." },
];
...
export function CardFormSheet({ card, baseCurrency, onOpenChange, open }: { card?: CardSummary; baseCurrency: CurrencyCode; onOpenChange: (open: boolean) => void; open: boolean }) {
  ...
  async function submit(input: CardFormInput) {
    try {
      if (parseMoneyInput(input.credit_limit) < 0n || parseMoneyInput(input.bank_balance) < 0n || comparable < 0n) {
        form.setError("root", { message: "Los importes de la tarjeta y su punto de inicio deben ser válidos." }); return;
      }
  ...
  return <ResponsiveDialog description={card ? "Los cambios se aplicarán hacia adelante y no alterarán tu historial." : "Agrega los datos de tu tarjeta y el saldo que muestra tu banco."} footer={<><Button onClick={() => onOpenChange(false)} type="button" variant="ghost">Cancelar</Button><Button disabled={mutation.isPending} form="card-form" type="submit">{mutation.isPending ? "Guardando…" : card ? "Guardar cambios" : "Agregar tarjeta"}</Button></>} onOpenChange={onOpenChange} open={open} size="large" title={card ? "Editar tarjeta" : "Nueva tarjeta"}>
    <form id="card-form" onSubmit={(event) => void form.handleSubmit(submit)(event)}><div className="space-y-8">
      ...
      {card ? <section className="rounded-2xl bg-surface-secondary p-5"><p className="text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Inicio del seguimiento</p><p className="mt-3 text-sm font-semibold">En Nexo desde el {formatFinancialDate(card.baseline_date, "dd 'de' MMMM 'de' yyyy")}</p><p className="mt-1 text-xs leading-relaxed text-muted-foreground">Los movimientos anteriores se conservan como referencia, pero no vuelven a cambiar el saldo.</p></section> : null}
      {!card ? <section><h3 className="text-lg font-semibold">¿Desde cuándo quieres llevar esta tarjeta en Nexo?</h3><div className="mt-4 grid gap-3 sm:grid-cols-3">{baselineOptions.map((option) => <button className={cn("rounded-2xl border p-4 text-left transition", policy === option.value ? "border-primary bg-primary-soft" : "border-border hover:border-primary/30")} key={option.value} onClick={() => form.setValue("baseline_policy", option.value, { shouldValidate: true })} type="button"><span className="text-sm font-semibold">{option.title}</span><span className="mt-1 block text-xs leading-relaxed text-muted-foreground">{option.description}</span></button>)}</div><div className="mt-5 grid gap-5 sm:grid-cols-2"><FormField error={form.formState.errors.baseline_date?.message} id="baseline-date" label={policy === "specific_date" ? "Fecha de inicio" : "Fecha del saldo"}><Input id="baseline-date" type="date" {...form.register("baseline_date")} /></FormField><FormField error={form.formState.errors.bank_balance?.message} id="bank-balance" label="Saldo utilizado que muestra tu banco"><Input id="bank-balance" inputMode="decimal" {...form.register("bank_balance")} /></FormField>{policy === "after_last_statement" ? <FormField error={form.formState.errors.excluded_statement_amount?.message} hint="El importe de un estado anterior que ya consideras resuelto." id="excluded-statement" label="Estado anterior ya resuelto"><Input id="excluded-statement" inputMode="decimal" {...form.register("excluded_statement_amount")} /></FormField> : null}<FormField error={form.formState.errors.baseline_notes?.message} id="baseline-notes" label="Notas (opcional)"><Input id="baseline-notes" {...form.register("baseline_notes")} /></FormField></div>{policy === "after_last_statement" ? <div className="mt-5 rounded-2xl border border-border p-5"><p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Así empezará la tarjeta</p><div className="mt-3 flex justify-between text-sm"><span>Saldo del banco</span><MoneyValue amount={bankBalance} currency={form.watch("currency")} /></div><div className="mt-2 flex justify-between text-sm"><span>Menos el estado ya resuelto</span><MoneyValue amount={-excluded} currency={form.watch("currency")} sign="always" /></div><div className="mt-3 flex justify-between border-t border-border pt-3 font-semibold"><span>Saldo con el que empezarás</span><MoneyValue amount={comparable} currency={form.watch("currency")} /></div></div> : null}</section> : null}
