...
- **Rate limiting**: `private.shortcut_rate_limit_windows` + `private.check_shortcut_rate_limit(bucket_key, limit)` — ventana fija por minuto, incremento atómico vía `insert ... on conflict do update ... returning`, sin memoria de proceso ni Deno KV. Todavía **no está conectado** a `execute_shortcut_transaction`: la integración por token y el bucket por IP para tokens inválidos necesitan el contexto de request de una Edge Function real (7C-B).
