...
## 15. Estado después de Fase 7C-A

**Solo dominio y autenticación SQL. Sin Edge Function, sin `supabase/functions`, sin UI de Integraciones, sin instrucciones de Atajos, sin `service_role` real, sin deploy, sin aplicar a Supabase real — todo eso queda para 7C-B.**

Implementado: extracción sin cambio de comportamiento de `create_transaction`/`create_card_purchase` en `private.create_transaction_for_user(p_user_id, ...)`/`private.create_card_purchase_for_user(p_user_id, ...)` (cuerpo idéntico, `command_user_id` pasa a ser un parámetro explícito en vez de `private.require_user()`); los RPC públicos quedan como envoltorios de una línea, misma firma/grants/comportamiento/idempotencia, verificado corriendo la suite completa de tests SQL preexistente sin modificar una sola aserción. `shortcut_tokens` (token `nexo_shortcut_<43 base64url>`, 32 bytes generados en servidor, solo `sha256` persistido, texto plano devuelto una única vez, scopes `shortcut:options:read`/`shortcut:transactions:write` reales desde ya). RPCs de gestión `create_shortcut_token`/`list_shortcut_tokens`/`revoke_shortcut_token` (`list` nunca expone hash ni plano). `private.resolve_shortcut_token` (hash entra, identidad mínima sale, nunca acepta texto plano). `public.execute_shortcut_transaction`, `service_role`-only, único punto de entrada de ejecución — despacha exclusivamente a las `*_for_user`, nunca escribe el ledger por su cuenta, ownership validado dentro de esas mismas funciones contra el `user_id` resuelto del token. `private.shortcut_token_usage` (auditoría separada de `financial_commands`, upsert por `unique(token_id, shortcut_execution_id)` para que un reintento nunca multiplique ni degrade una fila que ya decía éxito). `private.shortcut_rate_limit_windows` + `private.check_shortcut_rate_limit` (ventana fija por minuto, atómica, todavía sin conectar a ningún punto de entrada).

Corrección de diseño encontrada y cerrada durante esta misma fase (no documentada como limitación conocida): la primera versión de `execute_shortcut_transaction` capturaba un rechazo de dominio (cuenta ajena, archivada, categoría inválida, conflicto de idempotencia), insertaba la fila de auditoría de error, y volvía a lanzar la excepción original para que el llamador viera el código real — pero relanzar una excepción no capturada aborta toda la transacción que envuelve la llamada RPC, y ese aborto se llevaba consigo la fila de auditoría recién insertada, dejando cero evidencia de errores genuinos. Se descubrió al escribir el propio caso de prueba (CASO Q/U) antes de dar la fase por cerrada. Corregido cambiando el tipo de retorno de `uuid` a una fila `(ok boolean, event_id uuid, error_code text)`: un rechazo de dominio con token ya resuelto se audita y se **retorna** como `ok=false`, nunca se relanza; solo los rechazos anteriores a resolver el token (sin `token_id` contra el cual auditar nada) siguen lanzando de forma dura. Verificado con CASO Q/U corriendo limpio tras el cambio.

`src/lib/money-core.ts`: núcleo puro decimal↔minor-units extraído de `src/lib/money.ts`, cero imports (ni alias `@/`, ni `CurrencyCode`), listo para ser importado sin cambios desde una Edge Function Deno en 7C-B. `money.ts` reexporta todo desde ahí y conserva únicamente `formatMoney` (UI-only); las 33 ubicaciones existentes que importan `@/lib/money` no cambiaron ni una línea. Pruebas nuevas en `money-core.test.ts`, pruebas existentes de `money.test.ts` intactas.

Pruebas permanentes incorporadas (`supabase/tests/shortcut_tokens.test.sql`, casos A-Z): extracción sin regresión de `create_transaction`/`create_card_purchase` (A, B), ownership enforced dentro de `*_for_user` directamente (C), ciclo de vida del token (D-J: válido, inválido, revocado, expirado, scope insuficiente, `list` nunca expone secretos, revocar uno no afecta otro), ejecución correcta contra cuenta y tarjeta con auditoría coherente (K, L, V), rechazo de cuenta/tarjeta ajena o archivada y categoría inválida sin dejar rastro en el ledger (M-Q), idempotencia con payload igual/distinto y auditoría no multiplicada ni degradada (R, S, T, Y5), rollback total ante fallo financiero (U), movimientos previos intactos tras revocar (W), `execute_shortcut_transaction` inalcanzable para `anon`/`authenticated` (Y2), primitiva de rate limit con ventana y aislamiento entre ventanas (X, Y), y verificación cruzada del hash sha256 contra un cómputo independiente (Y6). Toda la suite preexistente (`accounts_movements_rls`, `card_transactions_rls`, `budgets`, `goals`, `financial_planning`, `recurring_transactions`, checkpoint de 7A, etc.) corrió sin cambios y sin fallos sobre el refactor.

Pendiente para 7C-B, explícitamente fuera de esta fase: Edge Function real (`GET /shortcut/options`, `POST /shortcut/transactions`), integración del rate limit (por token y por IP para tokens inválidos) en el punto de entrada HTTP, UI de Configuración → Integraciones → Atajos de iPhone, instrucciones del Atajo de iOS, y aplicación de la migración `20260901000000_phase_7c_a_shortcuts_domain.sql` a Supabase real.

## 16. Estado después de Fase 7C-B

**Capa HTTP real. Sin UI de Integraciones, sin Atajo de iOS real, sin despliegue, sin secretos reales — eso es 7C-C/7C-D.**

Implementado: `supabase/functions/shortcut-options/index.ts` (GET, lista opciones) y `supabase/functions/shortcut-transactions/index.ts` (POST, registra una compra), más helpers compartidos en `supabase/functions/_shared/` (`auth.ts`, `http.ts`, `errors.ts`, `rate-limit.ts`, `date.ts`, `validation.ts`, `money.ts`, `supabase-admin.ts`, `cors.ts`). Migración SQL nueva `20260902000000_phase_7c_b_shortcut_options_and_rate_limit.sql`: `public.get_shortcut_options(token_hash)`, `public.get_shortcut_transaction_context(token_hash, source_type, source_id)` (moneda real + timezone en una sola llamada, bajo scope `transactions:write`), `public.check_shortcut_abuse_bucket(bucket_key, limit)` (wrapper genérico del rate limiter de 7C-A), y `execute_shortcut_transaction` con el rate limit ya conectado (`'tx:'`/`'options:'`, 30/min cada uno).

Bug real encontrado y corregido durante esta fase (no documentado como limitación): `get_shortcut_transaction_context` fallaba con "column reference \"currency\" is ambiguous" porque su propia columna de retorno se llama `currency`, colisionando con `accounts.currency`/`credit_cards.currency` sin alias — el mismo tipo de bug de nombres-que-colisionan que ya había aparecido varias veces antes en el proyecto. Se detectó al correr el harness de HTTP local real (no en una revisión de código), y se corrigió calificando las columnas con alias de tabla.

Autenticación: sin JWT de impersonación (decisión ya cerrada antes de 7C-A, respetada aquí). El Atajo manda únicamente `Authorization: Bearer nexo_shortcut_...`; la Edge Function calcula `sha256` con Web Crypto y solo envía el hash a Postgres — verificado por lectura de código y por el harness HTTP real (ningún log, ninguna respuesta de error, incluye el token). `service_role` vive solo en `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY`, leídas en un único archivo (`_shared/supabase-admin.ts`); la Edge Function nunca hace `select`/`insert` directo sobre una tabla financiera.

Money-core: `src/lib/money-core.ts` (creado en 7C-A) se reexporta sin copiar desde `supabase/functions/_shared/money.ts` vía ruta relativa — confirmado con `deno check` (pasa) y `deno eval` ejecutando el parser real: `parseMoneyInput("450.50", 2)` → `45050n`, el mismo resultado que produce el frontend.

Fecha/timezone: `resolveLocalDate` usa `Intl.DateTimeFormat("en-CA", { timeZone })`, verificado empíricamente con Deno cruzando una frontera de medianoche real (05:30 UTC del 2 de septiembre = 1 de septiembre en `America/Mexico_City`) — nunca `toISOString().slice(0,10)`.

Rate limit de tokens inválidos: se auditó qué cabecera de IP es realmente confiable en Supabase Edge Functions antes de decidir (ver DOMAIN_RULES.md §41) — `cf-connecting-ip` (fijada por Cloudflare, no falsificable por el cliente) cuando está presente; un único bucket global de reserva, documentado como limitación real, cuando no lo está (por ejemplo en desarrollo local). Nunca `x-forwarded-for` como fuente de verdad.

Concurrencia real, pendiente cerrado de 7C-A: `scripts/test-shortcut-concurrency.sh` (`npm run test:shortcut-concurrency`) — dos procesos `psql` genuinamente concurrentes, mismo `shortcut_execution_id`, mismo payload, contra un Postgres bare efímero propio; 30/30 intentos produjeron exactamente un `financial_event` y una fila de `shortcut_token_usage`. No depende de Supabase real.

Validación HTTP local real, no solo compilación: `supabase functions serve` se intentó primero y falla de inmediato en este entorno (requiere Docker/Podman — confirmado ausente vía el propio mensaje de error de la CLI, no asumido). `scripts/test-shortcut-http-local.sh` (`npm run test:shortcut-http-local`) construye la alternativa más cercana sin Docker: Postgres bare + PostgREST real (instalado vía Homebrew) + un proxy `/rest/v1` mínimo (`scripts/_rest_v1_proxy.ts`, necesario porque `@supabase/supabase-js` asume ese prefijo de gateway que un PostgREST desnudo no tiene) + los archivos reales de `supabase/functions/*/index.ts` corridos con `deno run --allow-net --allow-env --allow-read`. Contra eso: `curl` real verificando 200 (opciones válidas, sin balances/límites), 401 (token inválido/ausente), 405 (método incorrecto), 201 (compra registrada), reintento idempotente (mismo `transaction_id`), 409 (conflicto de idempotencia), 400 (categoría inválida, UUID malformado), y la conversión exacta "450.50" → 45050 unidades menores verificada contra la fila real de `account_entries`. Todo pasó.

Pruebas SQL permanentes nuevas: `supabase/tests/shortcut_http_layer.test.sql` — forma exacta de `get_shortcut_options` sin balances/límites, cuentas/tarjetas archivadas nunca aparecen, token de solo-escritura rechazado para `options:read`, `get_shortcut_transaction_context` devuelve moneda/timezone correctos y rechaza cuentas ajenas/archivadas, `check_shortcut_abuse_bucket` como primitiva genérica, y el rate limit genuinamente conectado tanto en `get_shortcut_options` como en `execute_shortcut_transaction` (este último nunca escribe `shortcut_token_usage` cuando el límite se excede).

Pendiente para 7C-C/7C-D, explícitamente fuera de esta fase: UI de Configuración → Integraciones → Atajos de iPhone, instrucciones/creación del Atajo real en iOS, despliegue real de las Edge Functions, secretos reales, y aplicación de ambas migraciones (7C-A y 7C-B) a Supabase real.

## Fase 7C — hoja de ruta completa

- **7C-A** — dominio y autenticación (SQL): `shortcut_tokens`, extracción `private.*_for_user`, `execute_shortcut_transaction`. Terminada, commiteada.
- **7C-B** — capa HTTP (Edge Functions): `shortcut-options`, `shortcut-transactions`, RPCs `get_shortcut_options`/`get_shortcut_transaction_context`/`check_shortcut_abuse_bucket`, rate limit conectado. Terminada, commiteada.
- **7C-C** — UI de Integraciones dentro de Nexo (esta fase): crear/listar/copiar/revocar accesos desde Configuración → Integraciones → Atajos de iPhone, sin desplegar ni configurar el Atajo real todavía. Ver estado detallado abajo.
- **7C-D** — pendiente: despliegue real de las Edge Functions, configuración de secretos reales en Supabase, aplicación de las migraciones 7C-A/7C-B a Supabase real, construcción e instrucciones del Atajo real en iOS.

**Fase 7C todavía NO está completa** — falta 7C-D en su totalidad.

## 17. Estado después de Fase 7C-C

**Solo UI de gestión dentro de Nexo. Sin despliegue, sin Atajo real, sin secretos reales — eso es 7C-D.**

Implementado dentro de Configuración (sin ruta nueva, sin entrada de sidebar nueva): una sección "Integraciones" → "Atajos de iPhone" en `src/features/settings/settings-page.tsx`, con listado de accesos (`list_shortcut_tokens`), creación en dos pasos (nombre → revelar token una sola vez con copiar), y revocación con confirmación (`revoke_shortcut_token`). Trío `src/schemas/shortcut-token.ts` + `src/services/shortcut-token-service.ts` + `src/hooks/use-shortcut-tokens.ts`, siguiendo exactamente el patrón ya establecido (`contactService`/`use-contacts.ts`) — sin idempotency key en `create` (decisión de 7C-A: un reintento nunca puede recuperar el texto plano), con idempotency key en `revoke`.

El token plano vive únicamente en estado local de React (`useState` dentro de `CreateShortcutTokenDialog`), nunca en la cache de TanStack Query más allá de la vida de la mutación (`create.reset()` se llama explícitamente al cerrar) — confirmado que este proyecto no usa ningún persister de TanStack Query (`src/app/query-client.ts` es un `QueryClient` en memoria puro, sin `persistQueryClient`/localStorage/IndexedDB de por medio), así que ni siquiera la cache interna de la mutación sobrevive a un refresh de página. Nunca se llama `console.log`/`reportError` con la respuesta de `create_shortcut_token`.

Bug real encontrado y corregido durante esta fase (no cosmético): el nuevo helper `formatRelativeTime` (agregado a `src/lib/dates.ts`) usaba `isToday`/`isYesterday` de `date-fns`, que ignoran por completo el parámetro `now` inyectable y siempre comparan contra el reloj real del sistema — haciendo la función efectivamente no testeable y, peor, silenciosamente incorrecta para cualquier `now` que no fuera el instante real. Se detectó al escribir la prueba correspondiente (no en revisión de código) y se corrigió comparando el día calendario local directamente contra `now`.

RPCs usadas desde el navegador: exclusivamente `create_shortcut_token`, `list_shortcut_tokens`, `revoke_shortcut_token` — las tres ya registradas en `database.ts` desde 7C-A. Ninguna de las RPC `service_role`-only (`execute_shortcut_transaction`, `get_shortcut_options`, `get_shortcut_transaction_context`, `check_shortcut_abuse_bucket`) se referencia desde `src/` — verificado no solo por diseño sino con una prueba automatizada que escanea todo `src/**/*.{ts,tsx}` buscando esos cuatro nombres entre comillas (la forma que tomaría una llamada real) y falla si aparece alguno.

Validación visual: no se pudo hacer un recorrido real en navegador autenticado, porque esta fase prohíbe explícitamente Supabase real y el entorno local no tiene un servidor de Auth (GoTrue) — solo Postgres+PostgREST bare, sin login real posible. La verificación se hizo por: build/typecheck limpios (confirman que el árbol de componentes es válido), y pruebas de componente reales con Testing Library (`@testing-library/react`, ya usado en el proyecto) que renderizan `ShortcutTokensSection`/`CreateShortcutTokenDialog` con el servicio mockeado y confirman el DOM real — vacío, activo, revocado, revelar/copiar/cerrar el token, confirmación antes de cerrar, mensajes de error exactos. El comportamiento responsive no se verificó con captura de pantalla; se apoya en reutilizar sin modificar los mismos componentes (`ResponsiveDialog`/`Modal`, `ConfirmDialog`) que ya son responsive en producción en el resto de la app.

Pendiente para 7C-D, explícitamente fuera de esta fase: desplegar `shortcut-options`/`shortcut-transactions` de verdad, configurar `SUPABASE_SERVICE_ROLE_KEY` real como secreto de la Edge Function, aplicar `20260901000000_phase_7c_a_shortcuts_domain.sql` y `20260902000000_phase_7c_b_shortcut_options_and_rate_limit.sql` a Supabase real, construir el Atajo real de iOS y completar el modal "Ver cómo configurar" con la dirección real del servicio (deliberadamente no inventada todavía).
