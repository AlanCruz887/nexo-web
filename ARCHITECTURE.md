...
### 3.9 Integraciones — Atajos de iPhone (dominio en 7C-A, capa HTTP en 7C-B; UI e instrucciones del Atajo todavía pendientes)

| Entidad | Responsabilidad |
|---|---|
| `shortcut_tokens` (implementada, Fase 7C-A) | Token personal revocable y escopado para captura rápida; solo `sha256(token)` se persiste, el valor plano se devuelve una única vez al crearlo y nunca vuelve a ser consultable |
| `private.shortcut_token_usage` (implementada, Fase 7C-A) | Auditoría de cada intento de ejecución vía token — separada de `financial_commands` a propósito, para no alterar la semántica de idempotencia del motor real |
| `private.shortcut_rate_limit_windows` (implementada, Fase 7C-A; conectada en 7C-B) | Ventanas fijas de un minuto para limitar solicitudes por token, atómicas vía `insert ... on conflict do update` |
| `public.get_shortcut_options`, `public.get_shortcut_transaction_context`, `public.check_shortcut_abuse_bucket` (implementadas, Fase 7C-B) | Tres RPC estrechas `service_role`-only adicionales: opciones para los selectores del Atajo, moneda real + timezone del usuario antes de ejecutar, y el primitivo de rate limit genérico reutilizado para tokens inválidos |
| `supabase/functions/shortcut-options`, `supabase/functions/shortcut-transactions` (implementadas, Fase 7C-B) | Las dos Edge Functions — nunca escriben el ledger directo, despachan exclusivamente a las RPC anteriores |

Fase 7C-A agregó el dominio SQL y la extracción de `create_transaction`/`create_card_purchase` en `private.*_for_user(user_id, ...)`. Fase 7C-B construye encima la capa HTTP: dos Edge Functions Deno (`GET /shortcut-options`, `POST /shortcut-transactions`) más los helpers compartidos en `supabase/functions/_shared/` (auth, http, errors, rate-limit, date, validation, money, supabase-admin) — sin UI de Nexo ni instrucciones del Atajo de iOS todavía, eso es 7C-C/7C-D.
...
Fase 7C-A prepara el dominio de captura rápida por Atajos sin agregar ninguna Edge Function todavía. `create_transaction`/`create_card_purchase` se extraen a `private.create_transaction_for_user(p_user_id, ...)`/`private.create_card_purchase_for_user(p_user_id, ...)` — el cuerpo financiero es idéntico, solo cambia de dónde viene `command_user_id` (de `private.require_user()` a un parámetro explícito); los RPC públicos quedan como envoltorios de una línea. Es la misma técnica que 6B ya usó para separar lógica reutilizable de su resolución de identidad, aplicada ahora al núcleo de movimientos porque un futuro llamador (el Atajo, vía token, nunca vía sesión de Supabase) necesita ejecutar exactamente la misma operación para un usuario que no es `auth.uid()` del llamador real. `public.execute_shortcut_transaction` es el único punto nuevo que conoce ambos mundos: resuelve el token, y despacha a esas mismas funciones — nunca escribe el ledger por su cuenta, y el ownership de la cuenta/tarjeta se valida dentro de ellas, no en una capa paralela.

La decisión de diseño más delicada de 7C-A es que `execute_shortcut_transaction` **retorna** `(ok, event_id, error_code)` en vez de lanzar una excepción cuando el motor financiero rechaza la operación (cuenta ajena, archivada, categoría inválida, conflicto de idempotencia). La razón es puramente transaccional: una excepción no capturada aborta toda la transacción que envuelve la llamada RPC (en producción, la transacción implícita de una sola petición HTTP), y ese aborto se llevaría consigo la fila de auditoría de error que el manejador de excepción interno acababa de insertar — dejando la auditoría exactamente incoherente con lo ocurrido. Se descubrió empíricamente durante la propia validación de 7C-A (un primer diseño que sí relanzaba la excepción dejaba `shortcut_token_usage` sin fila alguna para el caso de error) y se corrigió antes de cerrar la fase, no se documentó como limitación conocida. Solo los rechazos anteriores a resolver el token (tipo de fuente inválido, `shortcut_execution_id` nulo, o el propio `private.resolve_shortcut_token`) siguen lanzando de forma dura, porque ahí no existe todavía ningún `token_id` contra el cual auditar nada.

`shortcut_tokens` nunca guarda el token en texto plano — solo `sha256(token)`, calculado del lado servidor con `extensions.gen_random_bytes(32)`/`extensions.digest(...)` dentro de la propia RPC de creación. `private.shortcut_token_usage` es una auditoría deliberadamente separada de `financial_commands`, para no arriesgar la semántica de idempotencia del motor real con metadata ajena a la operación financiera; a diferencia de `audit_events`, se actualiza por upsert sobre `unique(token_id, shortcut_execution_id)` para que un reintento nunca multiplique ni incoherentemente regrese una fila que ya registraba éxito. `private.shortcut_rate_limit_windows` + `private.check_shortcut_rate_limit` quedan como primitiva atómica de Postgres (ventana fija por minuto, `insert ... on conflict do update ... returning`), probada de forma independiente pero todavía sin conectar a ningún punto de entrada — esa integración, junto con el bucket por IP para tokens inválidos, necesita el contexto de una Edge Function real y queda para Fase 7C-B.

Fase 7C-B construye la capa HTTP encima del dominio de 7C-A, sin reabrir ninguna de sus decisiones. Dos Edge Functions (`supabase/functions/shortcut-options`, `supabase/functions/shortcut-transactions`) más los helpers en `supabase/functions/_shared/`. El flujo es exactamente el auditado antes de aprobar 7C-A: Atajo → HTTPS → Edge Function → `sha256` del token calculado en Deno (nunca en Postgres) → RPC estrecha `service_role`-only → `private.*_for_user` → respuesta humana. **Sin JWT de impersonación en ningún punto** — esa alternativa ya se descartó formalmente antes de 7C-A y esta fase no la reconsidera. Dos RPC nuevas cierran el resto del contrato: `get_shortcut_transaction_context(token_hash, source_type, source_id)` resuelve en una sola llamada la moneda real de la fuente y el `timezone` del perfil, bajo el scope `transactions:write` (no `options:read`, para que un token de solo-escritura pueda completar una compra sin necesitar permiso de lectura); `get_shortcut_options` sigue exactamente el contrato ya auditado (`{accounts, cards, categories}`, sin balances ni límites). El parser de dinero es una sola implementación real, no una promesa: `supabase/functions/_shared/money.ts` reexporta `src/lib/money-core.ts` por ruta relativa, y `deno check`/`deno eval` lo ejecutaron de verdad contra ese archivo exacto durante la validación de esta fase (no una copia, no una reimplementación).

El rate limit de tokens inválidos expone la primera decisión genuinamente nueva de 7C-B: qué señal de origen usar cuando no hay `token_id` todavía. Se auditó la documentación/discusiones oficiales de Supabase antes de decidir — `x-forwarded-for` es spoofeable por el cliente (puede anteponer cualquier valor); `cf-connecting-ip` es la única cabecera que Cloudflare (la capa que efectivamente antecede a las Edge Functions de Supabase) fija por sí misma y que un cliente no puede sobrescribir. Cuando ninguna está presente (desarrollo local sin Cloudflare, por ejemplo), la función cae a un único bucket global compartido — una limitación real, documentada como tal en el código y en `DOMAIN_RULES.md` §41, no una simulación de seguridad por IP que en realidad no puede distinguir clientes.

La verificación de esta fase no se detuvo en "TypeScript compila". `supabase functions serve` se intentó primero y falla de inmediato en este entorno (requiere Docker/Podman, confirmado ausente, no asumido). En su lugar: `scripts/test-shortcut-concurrency.sh` levanta un Postgres bare efímero (misma técnica que `scripts/test-db.sh`) y lanza dos procesos `psql` genuinamente concurrentes contra `execute_shortcut_transaction` con el mismo `shortcut_execution_id` — 30/30 intentos produjeron exactamente un `financial_event`, cerrando el pendiente de concurrencia real dejado abierto por 7C-A. `scripts/test-shortcut-http-local.sh` va más lejos: Postgres real + PostgREST real (instalado vía Homebrew) + un proxy mínimo que traduce `/rest/v1/*` (la convención de `@supabase/supabase-js`) al root de PostgREST + los archivos reales de `supabase/functions/*/index.ts` corridos con `deno run` — y contra eso, peticiones `curl` reales verificando 200/401/404/405/409/400 y la conversión exacta de "450.50" a 45050 unidades menores contra la fila real de `account_entries`. Ninguno de los dos scripts depende de Docker ni de Supabase real.
