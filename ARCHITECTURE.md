...
### 3.9 Integraciones — Atajos de iPhone (dominio preparado en Fase 7C-A, sin capa HTTP todavía)
...
| `private.shortcut_rate_limit_windows` (implementada, Fase 7C-A) | Ventanas fijas de un minuto para limitar solicitudes por token, atómicas vía `insert ... on conflict do update`; primitiva SQL lista, todavía no conectada a ningún punto de entrada |

Fase 7C-A no agrega ninguna Edge Function, ruta HTTP ni pantalla — solo el dominio SQL y la extracción de `create_transaction`/`create_card_purchase` en `private.*_for_user(user_id, ...)`, reutilizada tanto por los RPC públicos existentes (sin cambio de comportamiento) como por el nuevo `public.execute_shortcut_transaction` (`service_role`-only). Ese es el único punto de entrada preparado para que una futura Edge Function (Fase 7C-B) reciba una captura desde el Atajo — hoy se prueba llamándolo directamente desde SQL, exactamente como lo haría esa función más adelante.
