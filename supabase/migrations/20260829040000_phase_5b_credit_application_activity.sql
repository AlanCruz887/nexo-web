-- Nexo Phase 5B correction: surface saldo a favor applications in the person's
-- own activity feed with the right label. person_credit_application events do
-- reduce receivable_entries (same as any payment), so contact_activity already
-- picked them up, but only through the generic branch that labels anything
-- that is not a person_payment as a "purchase" — the opposite of what
-- happened. This excludes them from that branch and adds a dedicated one
-- sourced from receivable_due_applications, the same ledger
-- get_person_collection_period already reads, so no new balance engine is
-- introduced.
...
-- Same correction: each period concept already carries its installment
-- number (sequence_number) but not the plan's total term, so the UI could
-- not present "Mensualidad 2 de 12" without asking a second endpoint or
-- redoing the schedule math itself. This adds installment_count to the
-- existing jsonb payload; the eligibility/period-selection logic is
-- untouched.
